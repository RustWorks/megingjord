...
impl Task {
    pub fn new(client: Client, action: TaskAction) -> Self {
        ...
        std::thread::spawn(move || runtime.block_on(async move { Task::dispatch(client, action, result_tx).await }));
        #[cfg(target_arch = "wasm32")]
        runtime.spawn(async move { Task::thread_main(client, action, result_tx).await });
